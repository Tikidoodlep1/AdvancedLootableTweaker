package tiki.advancedlootabletweaker.blocks;
...
import tiki.advancedlootabletweaker.compat.contenttweaker.ForgeRepresentation;
...
//            	TEF2.setMainTE(TEF2);
	...
	{
		return new TileEntityForge2(false, false, this.forge.getBaseHeatingSpeed(), this.getLocalizedName(), this.getRegistryName());
	}
...
//    	}
    	...
    	if(BlockModelJson != null) {
        	...
        	if(BlockModelJson.size() > 0) {
        		...
        		tiki.advancedlootableweapons.Alw.logger.warn("Unable to generate block model for " + this.getUnlocalizedName() + ". Please check that you are using the latest version of ALW. If you are, please report this to the mod author!");
        	}
    	}
